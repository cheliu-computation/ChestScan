...
def data_prep(task, batch_size):
    ...
    class MIMIC_CheX_CXR8(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            if self.database == 'MIMIC':
                ...
                sharedlabel = ['Atelectasis', 'Cardiomegaly', 'Consolidation', 'Edema', 
                'Effusion', 'No Finding', 'Pneumonia', 'Pneumothorax']
                target = self.target_csv[sharedlabel].iloc[idx]
    ...
    if task == 'MIMIC':
        ...
        raw_train_dataset = np.load('/rds/general/user/cl522/home/X-scan/MIMIC/train_test_val/MIMIC_train.npy', mmap_mode='r')
        ...
        raw_train_dataset = np.load('/data/che/CheXpert/CheXpert-v1.0/CheXpert_train.npy', mmap_mode='r')
        train_csv = pd.read_csv('/data/che/CheXpert/CheXpert-v1.0/CheXpert_train.csv')

        raw_test_dataset = np.load('/data/che/CheXpert/CheXpert-v1.0/CheXpert_test.npy', mmap_mode='r')
        test_csv = pd.read_csv('/data/che/CheXpert/CheXpert-v1.0/CheXpert_test.csv')
        
    elif task == 'CXR8':
        print('CXR8 task loading')
        raw_train_dataset = np.load('/data/che/CXR8/DL_data/CXR8_train_8_label.npy', mmap_mode='r')
        train_csv = pd.read_csv('/data/che/CXR8/DL_data/train.csv')

        raw_test_dataset = np.load('/data/che/CXR8/DL_data/CXR8_test_8_label.npy', mmap_mode='r')
        test_csv = pd.read_csv('/data/che/CXR8/DL_data/test.csv')
